            ...
            for flavor1 in range(2):
                for flavor2 in range(2):
                    ...
                    for band in range(2*num_sub):
                        N_onsite += (ubov21[X1, band] * ubov21_c[X2, band])
...
# def contractions_bond(bond):
    ...
    # def integrand_Nij(ndim, xx, ncomp, ff, userdata):
        ...
        # for flavor1 in range(2):
            # for flavor2 in range(2):
                ...
                # X1i = num_sub*flavor1 + sub1
                # X1j = num_sub*flavor1 + sub2
                # X2i = num_sub*flavor2 + sub1
                # X2j = num_sub*flavor2 + sub2

                # Nii = (ubov11[X1i, :] * ubov11_c[X2i, :]).sum()
                # Njj = (ubov11[X1j, :] * ubov11_c[X2j, :]).sum()
                # Nij = (ubov11[X1i, :] * ubov11_c[X2j, :] * cphase).sum()
    ...
    # res = pycuba.Vegas(integrand_Nij, NDIM, epsabs=1e-3, \
